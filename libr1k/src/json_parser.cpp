...
int json_parser::to_buffer(const CBitrateEvent &bre)
{
	...
	message.push_back( json_spirit::Pair("pid", (uint16_t)bre.Pid));
	message.push_back( json_spirit::Pair("bitrate", (uint64_t)bre.Bitrate));
...
int json_parser::to_buffer(const CPidEvent &pe)
{
    ...
    message.push_back( json_spirit::Pair("pid", (uint16_t)pe.Pid));
